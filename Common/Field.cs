...
namespace Common
{
    public class Field<TYPE>
    {
        ...
        public void TransformSitePropose(int i, Point from, out Point to)
        {
            TransformSitePropose(i, from.X, from.Y, out to.X, out to.Y);
        }
        ...
        public IEnumerable<Point> Adjoin(Point point)
        {
            for (int i = 1; i < 7; i++)
            {
